  ...
  return (
    <div>
      ...
      >
        ...
        {/* Icon */}
        {node.type === 'year' && (
          <Calendar className={`w-4 h-4 flex-shrink-0 ${isSelected ? 'text-emerald-600 dark:text-emerald-400' : 'text-zinc-500'}`} />
        )}
        {node.type === 'week' && (
          isExpanded
            ? <FolderOpen className={`w-4 h-4 flex-shrink-0 ${isSelected ? 'text-emerald-600 dark:text-emerald-400' : 'text-amber-600 dark:text-amber-500'}`} />
            : <Folder className={`w-4 h-4 flex-shrink-0 ${isSelected ? 'text-emerald-600 dark:text-emerald-400' : 'text-amber-600/70 dark:text-amber-500/70'}`} />
        )}
        {isCategory && CategoryIcon && (
          <CategoryIcon className={`w-4 h-4 flex-shrink-0 ${isSelected ? 'text-emerald-600 dark:text-emerald-400' : 'text-zinc-500'}`} />
        )}
